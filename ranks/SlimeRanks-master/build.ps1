# SlimeRanks Build Script
Write-Host "Building SlimeRanks..." -ForegroundColor Cyan
...
if (-not $mavenCmd) {
    ...
    if (-not (Test-Path $mavenZip)) {
        Write-Host "Downloading Maven from $mavenUrl..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "`nBuild successful! JAR file is in the target directory." -ForegroundColor Green
    
    # Show JAR file location
    $jarFile = Get-ChildItem -Path "target" -Filter "SlimeRanks-*.jar" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
