...
public class FarmPlacerListener implements Listener {
    
    private final FarmShop plugin;
    
    public FarmPlacerListener(FarmShop plugin) {
        this.plugin = plugin;
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        // Check if this is a farm placer item
        String farmKey = null;
        for (String line : lore) {
            if (line.contains("Farm: ")) {
                farmKey = line.replace(ChatColor.GRAY.toString(), "").replace("Farm: ", "").trim();
                break;
            }
        }
        ...
        FarmManager.FarmData farm = plugin.getFarmManager().getFarm(farmKey);
        if (farm == null) {
            return;
        }
        ...
        if (plugin.getProtectionManager() != null) {
            com.playpandora.farmshop.managers.ProtectionManager.ProtectionResult result = 
        ...
        // The paste will be placed at the player's location
        String command = "paste " + farm.getPasteId();
        ...
        try {
            ...
            if (success) {
                ...
                String prefix = plugin.getConfig().getString("messages.prefix", "&e&lPandora");
                String message = plugin.getConfig().getString("messages.farm-placed",
                    "{prefix} &eFarm placed successfully at your location!")
                    .replace("{prefix}", prefix);
                player.sendMessage(ChatColor.translateAlternateColorCodes('&', message));
            ...
            String message = plugin.getConfig().getString("messages.console-command-error",
                "{prefix} &7Error executing farm placement. Please contact an administrator.")
                .replace("{prefix}", prefix);
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', message));
