# Build script for ChatManager plugin
Write-Host "Building ChatManager plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "ChatManager-*.jar" | Select-Object -First 1
        if ($jarFile) {
