# Build script for CannonShop
Write-Host "Building CannonShop..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "CannonShop-*.jar" | Select-Object -First 1
        if ($jarFile) {
