# Build script for BuyGUI plugin
Write-Host "Building BuyGUI plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "BuyGUI-*.jar" | Select-Object -First 1
        if ($jarFile) {
