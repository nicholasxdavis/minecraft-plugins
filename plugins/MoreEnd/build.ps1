# Build script for MoreEnd plugin
Write-Host "Building MoreEnd plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "MoreEnd-*.jar" | Select-Object -First 1
        if ($jarFile) {
