...
public class IntegrationManager {
    ...
    public IntegrationManager(MoreEnd plugin) {
        this.plugin = plugin;
    ...
    public void giveKillXP(Player player, LivingEntity killed) {
        ...
        double endMultiplier = plugin.getConfig().getDouble("integrations.levelplugin.end-xp-multiplier", 2.0);
        double xp = baseXP * endMultiplier;
        ...
        try {
            ...
            if (hookPlugin != null && hookPlugin.isEnabled()) {
                ...
                hookAPI.getClass().getMethod("sendCustom", org.bukkit.entity.Player.class, 
                    ...
                    .invoke(hookAPI, player, "End Event!", eventName, 500, 3000, 1000);
            }
        } catch (Exception e) {
            // Silently fail
        }
    ...
    private double getBaseXPForMob(org.bukkit.entity.EntityType type) {
        switch (type) {
            case ENDERMAN: return 12.0;
            case SHULKER: return 25.0;
            case ENDERMITE: return 5.0;
            case PHANTOM: return 8.0;
            default: return 5.0;
        }
    }
}
