# Build script for MoreCaves plugin
Write-Host "Building MoreCaves plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "MoreCaves-*.jar" | Select-Object -First 1
        if ($jarFile) {
