...
public class FactionsEntityListener implements Listener {
    ...
    public void onEntityDeathHeadDrops(EntityDeathEvent event) {
        Entity entity = event.getEntity();
        
        // Prevent ender dragon and piglin head drops - NEVER drop heads regardless of circumstances
        if (entity.getType() == EntityType.ENDER_DRAGON) {
            event.getDrops().removeIf(drop -> {
                if (drop == null) return false;
                return drop.getType() == Material.DRAGON_HEAD;
            });
        }
        ...
        if (entity.getType() == EntityType.PIGLIN || entity.getType() == EntityType.PIGLIN_BRUTE) {
            event.getDrops().removeIf(drop -> {
                if (drop == null) return false;
                return drop.getType() == Material.PIGLIN_HEAD;
            });
        }
    }
    ...
    public void onEntityExplode(EntityExplodeEvent event) {
        ...
        // BUT: Allow explosions to break base structure blocks (walls/floors)
        // AND: Handle obsidian health system
        event.blockList().removeIf(block -> {
            // Always allow explosions to break base structure blocks
            if (com.massivecraft.factions.util.BaseStructureHelper.isBaseStructure(block)) {
                return false; // Don't remove - allow explosion (will be handled by obsidian health system)
            }
            
            // Handle obsidian health system
            if (block.getType() == Material.OBSIDIAN && Conf.obsidianHealthEnabled) {
                // Don't remove - let obsidian health system handle it
                return false;
            }
            
            // Normal explosion check for other blocks
            return !this.checkExplosionForBlock(boomer, block);
        });
    ...
    public void onBlockExplode(BlockExplodeEvent event) {
        // Allow TNT/explosives to break base structure blocks
        // Don't filter out base structure blocks from explosion
        event.blockList().removeIf(block -> {
            // Always allow explosions to break base structure blocks
            if (com.massivecraft.factions.util.BaseStructureHelper.isBaseStructure(block)) {
                return false; // Don't remove - allow explosion
            }
            // Normal explosion check for other blocks
            Faction faction = Board.getInstance().getFactionAt(FLocation.wrap(block.getLocation()));
            if (faction == null) return true;
            
            // Use same logic as entity explosions
            return !checkExplosionForBlock(null, block);
        });
    }
