...
public class CmdJoin extends FCommand {
    ...
    public void perform(CommandContext context) {
        FactionsPlugin.getInstance().getServer().getScheduler().runTaskAsynchronously(FactionsPlugin.instance, () -> {
            ...
            // Check base member limit first (5 max for bases)
            if (faction.hasBeacon() && faction.isNormal()) {
                int baseMemberLimit = 5;
                if (faction.getFPlayers().size() >= baseMemberLimit && !fplayer.isAdminBypassing()) {
                    context.msg(TL.COMMAND_JOIN_ATLIMIT_MEMBERS, faction.getTag(context.fPlayer), baseMemberLimit, fplayer.describeTo(context.fPlayer, false));
                    return;
                }
            }
            
            // Then check regular faction member limit
