...
public class CmdInvite extends FCommand {
    ...
    public void perform(CommandContext context) {
        ...
        // Check if this is a base faction (has beacon) and enforce 5 member limit
        if (context.faction.hasBeacon() && context.faction.isNormal()) {
            int currentMembers = context.faction.getFPlayers().size();
            int baseMemberLimit = 5;
            if (currentMembers >= baseMemberLimit) {
                context.msg(TL.COMMAND_JOIN_ATLIMIT_MEMBERS, context.faction.getTag(), baseMemberLimit, target.describeTo(context.fPlayer, false));
                return;
            }
        }
