...
public abstract class MemoryFPlayer implements FPlayer {
    ...
    public boolean canClaimForFactionAtLocation(Faction forFaction, FLocation flocation, boolean notifyFailure) {
        ...
        } else if (forFaction.isNormal() && forFaction.hasBeacon()) {
            // Factions with beacons (obi cubes) can ONLY claim the initial 100x100 area (81 chunks max)
            // They cannot claim any additional land beyond the 81 chunks from beacon placement
            // CLAIM_RADIUS_CHUNKS = 4 means from -4 to +4 = 9 chunks in each direction = 81 chunks total
            int maxChunksForBase = 81; // 9x9 chunks = 100x100 blocks (approximately)
            if (ownedLand >= maxChunksForBase) {
                error = TextUtil.parse("&cYour base can only claim 100x100 blocks (81 chunks). You cannot claim additional land!");
            }
            // Allow reclaiming within the 81 chunk limit if they lost some chunks
    ...
    public void sendMessage(String msg) {
        ...
        Player player = this.getPlayer();
        if (player == null) return;
        // Replace color codes: &a->&e, &2->&6, &f->&7
        msg = com.massivecraft.factions.util.ColorCodeReplacer.replaceColorCodes(msg);
