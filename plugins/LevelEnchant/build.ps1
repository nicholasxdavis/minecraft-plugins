# Build script for LevelEnchant plugin
Write-Host "Building LevelEnchant plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "LevelEnchant-*.jar" | Select-Object -First 1
        if ($jarFile) {
