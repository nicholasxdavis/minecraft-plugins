# Build script for HomeBuffs
Write-Host "Building HomeBuffs..."
...
if ($LASTEXITCODE -eq 0) {
    ...
    $jarFile = Get-ChildItem -Path "target" -Filter "HomeBuffs-*.jar" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    
    if ($jarFile) {
        $currentDir = Get-Location
        $parentDir = Split-Path -Parent $currentDir
        $pluginsPath = Join-Path $parentDir "plugins"
        Copy-Item -Path $jarFile.FullName -Destination (Join-Path $pluginsPath "HomeBuffs.jar") -Force
        Write-Host "Plugin installed to plugins\HomeBuffs.jar"
