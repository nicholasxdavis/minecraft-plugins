# Build script for SellGUI plugin
Write-Host "Building SellGUI plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "SellGUI-*.jar" | Select-Object -First 1
        if ($jarFile) {
