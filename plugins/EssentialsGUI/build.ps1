# Build script for EssentialsGUI
Write-Host "Building EssentialsGUI..."
...
if ($LASTEXITCODE -eq 0) {
    ...
    $jarFile = Get-ChildItem -Path "target" -Filter "EssentialsGUI-*.jar" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    
    if ($jarFile) {
        $currentDir = Get-Location
        $parentDir = Split-Path -Parent $currentDir
        $pluginsPath = Join-Path $parentDir "plugins"
        Copy-Item -Path $jarFile.FullName -Destination (Join-Path $pluginsPath "EssentialsGUI.jar") -Force
        Write-Host "Plugin installed to plugins\EssentialsGUI.jar"
