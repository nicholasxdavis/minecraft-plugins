...
public class WarpCommand implements CommandExecutor {
    ...
    public WarpCommand(EssentialsGUI plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0 || (args.length == 1 && (args[0].equalsIgnoreCase("open") || args[0].equalsIgnoreCase("gui")))) {
            ...
            plugin.getWarpGUI().openGUI(player);
            return true;
        }
        
        // For all other cases, delegate to Essentials by dispatching the command
        // This ensures Essentials commands like /warp list, /warp set, /warp <name>, etc. all work
        StringBuilder cmdBuilder = new StringBuilder("essentials:warp");
        for (String arg : args) {
