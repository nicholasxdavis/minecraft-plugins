# Build script for CraftAccess plugin
Write-Host "Building CraftAccess plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "CraftAccess-*.jar" | Select-Object -First 1
        if ($jarFile) {
