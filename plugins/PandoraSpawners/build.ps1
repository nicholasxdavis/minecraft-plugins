# Build script for PandoraSpawners plugin
Write-Host "Building PandoraSpawners plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "PandoraSpawners-*.jar" | Select-Object -First 1
        if ($jarFile) {
