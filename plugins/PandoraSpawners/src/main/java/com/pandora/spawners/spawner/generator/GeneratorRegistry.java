package com.pandora.spawners.spawner.generator;
...
public final class GeneratorRegistry implements Listener {
	...
	public static void initialize() {
		Bukkit.getPluginManager().registerEvents(new GeneratorRegistry(), PandoraSpawners.instance());
		load();
		retime(true);
	}
	...
	public static void retime(boolean first) {
		...
		active_task = PandoraSpawners.scheduler().runTimer(() -> {
			SPAWNERS.values().forEach(SpawnerWorld::tick);
			if(++t[0] > f) {
				t[0] = 0;
				SPAWNERS.values().forEach(SpawnerWorld::reduce);
			}
		}, first ? 20 : 5, Settings.settings.ticking_interval);
		offline();
	}
	...
	private static void offline() {
		...
		offline_task = PandoraSpawners.scheduler().runTimer(() -> {
