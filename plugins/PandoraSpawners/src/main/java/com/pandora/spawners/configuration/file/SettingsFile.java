...
import com.pandora.spawners.prices.PriceType;
...
public class SettingsFile extends AbstractFile {
	...
	protected void initialize() {
		...
		UpgradeType.stream().forEach(type -> {
			...
			// Hardcoded expensive money prices for upgrades (in economy currency)
			int basePrice = type.name().equals("RANGE") ? 500000 : 
			               type.name().equals("DELAY") ? 750000 : 1000000; // AMOUNT is most expensive
			file.addDefault("Modifiers.upgrades.prices.DEFAULT." + type.lower(), basePrice);
			int increase = basePrice / 2; // 50% increase per level
			file.addDefault("Modifiers.upgrades.price-increase.DEFAULT." + type.lower(), increase);
		});
		file.addDefault("Modifiers.upgrades.price-increase-type", IncreaseType.ADDITION.name());
		// Hardcode upgrade price type to ECONOMY (money)
		file.addDefault("Prices.upgrades.price-type", "ECONOMY");
		...
		file.addDefault("Prices.upgrades.level-cost", 1); // Level cost for upgrades (uses LevelPlugin)
		...
		if(c != null) {
			...
			c.comment("Spawners.reset-spawner-values",
					...
					"Useful if you want to remove PandoraSpawners and",
			...
			c.comment("Events.ignore-break-event",
					...
					"This means that PandoraSpawners will not add custom spawner",
			...
			c.comment("Modifiers.stacking.limit-permissions",
					...
					"  [ PandoraSpawners.stacking.permission.<name> ]");
			c.comment("Modifiers.stacking.limit-permissions.example",
					"This is an example permission, does not work in game.",
					"Player with permission (PandoraSpawners.stacking.permission.example)",
			...
			c.comment("Modifiers.breaking.unbreakable",
					...
					"  (PandoraSpawners.unbreakable.bypass) - this",
			...
			c.comment("Modifiers.breaking.chance-permissions",
					...
					"    <name>: <chance>",
					"Replace <name> with the specific permission name.",
					"  [ PandoraSpawners.breaking.permission.<name> ]");
			c.comment("Modifiers.breaking.chance-permissions.example",
					"This is an example permission, does not work in game.",
					"Player with permission (PandoraSpawners.breaking.permission.example)",
			...
			c.comment("Modifiers.players.owned.limit-permissions",
					...
					"    <name>: <limit>",
					"Replace <name> with the specific permission name.",
					"  [ PandoraSpawners.ownership.permission.<name> ]");
			c.comment("Modifiers.players.owned.limit-permissions.example",
					"This is an example permission, does not work in game.",
					"Player with permission (PandoraSpawners.ownership.permission.example)",
