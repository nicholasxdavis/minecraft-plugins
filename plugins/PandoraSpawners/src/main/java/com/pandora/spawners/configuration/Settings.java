...
import com.pandora.spawners.api.configuration.IFileValues;
...
import com.pandora.spawners.api.spawner.location.ISelector.Selection;
import com.pandora.spawners.configuration.Configuration.CF;
import com.pandora.spawners.prices.Group;
import com.pandora.spawners.prices.IncreaseType;
import com.pandora.spawners.prices.Price;
import com.pandora.spawners.prices.PriceManager;
...
public final class Settings {
	...
	protected void reload0() {
		...
		// Hardcoded disabled spawners - only allow specific spawners for economy balance
		// ALLOWED: WITHER_SKELETON, SHULKER, GUARDIAN, ENDERMAN, BLAZE, EVOKER, VILLAGER,
		//          SKELETON, ZOMBIE_VILLAGER, WITCH, PIGLIN, ZOMBIFIED_PIGLIN, VINDICATOR,
		//          RAVAGER, IRON_GOLEM, BEE, SLIME, COW, SHEEP, SQUID, TURTLE, MUSHROOM_COW,
		//          WARDEN, WITHER
		spawner_disabled.addAll(RF.enumerates(SpawnerType.class, List.of(
			"ALLAY", "ARMADILLO", "ARMOR_STAND", "AXOLOTL", "BAT", "BOAT", "BOGGED", "BREEZE",
			"CAMEL", "CAT", "CAVE_SPIDER", "CHEST_BOAT", "CHICKEN", "COD", "COPPER_GOLEM",
			"CREEPER", "CREAKING", "DOLPHIN", "DONKEY", "DROWNED", "ELDER_GUARDIAN", "ENDERMITE",
			"ENDER_DRAGON", "EXPERIENCE_BOTTLE", "EXPERIENCE_ORB", "FOX", "FROG", "GHAST", "GIANT",
			"GLOW_SQUID", "GOAT", "HOGLIN", "HORSE", "HUSK", "ILLUSIONER", "LLAMA", "MAGMA_CUBE",
			"MINECART", "MINECART_CHEST", "MINECART_COMMAND", "MINECART_FURNACE", "MINECART_HOPPER",
			"MINECART_SPAWNER", "MINECART_TNT", "MULE", "OCELOT", "PANDA", "PARROT", "PHANTOM",
			"PIG", "PIGLIN_BRUTE", "PIG_ZOMBIE", "PILLAGER", "POLAR_BEAR", "PUFFERFISH", "RABBIT",
			"SALMON", "SILVERFISH", "SKELETON_HORSE", "SNIFFER", "SNOWMAN", "SPIDER", "STRAY",
			"STRIDER", "TADPOLE", "TRADER_LLAMA", "TROPICAL_FISH", "VEX", "WANDERING_TRADER",
			"WOLF", "ZOGLIN", "ZOMBIE", "ZOMBIE_HORSE"
		)));
		// Also add any from config file (in case admins want to disable more)
		...
		.forEach(key -> {
			...
			String perm = "PandoraSpawners.stacking.permission." + key;
		...
		.forEach(key -> {
			if(key.equals("example") == true) return;
			String perm = "PandoraSpawners.breaking.permission." + key;
		...
		.forEach(key -> {
			if(key.equals("example") == true) return;
			String perm = "PandoraSpawners.ownership.permission." + key;
	...
	public boolean ignored(Block block) {
		if(ignore_natural == true
			...
			if(data.has(DataManager.ownerKey(), org.bukkit.persistence.PersistentDataType.STRING) == false) {
