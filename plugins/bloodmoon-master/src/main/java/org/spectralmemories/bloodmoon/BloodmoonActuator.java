...
public class BloodmoonActuator implements Listener, Runnable, Closeable
{
    ...
    private void BroadcastBloodMoonWarningPlayer (Player player)
    {
        ...
        // Inventory loss message removed - we don't clear inventory on death
    ...
    public void onPlayerDeath (PlayerDeathEvent event)
    {
        ...
        if (configReader.GetExperienceLossConfig())
        {
            // Reset custom level system instead of vanilla XP
            try {
                org.bukkit.plugin.Plugin levelPlugin = Bloodmoon.GetInstance().getServer().getPluginManager().getPlugin("LevelPlugin");
                if (levelPlugin != null && levelPlugin.isEnabled()) {
                    try {
                        // Get LevelManager directly
                        Object levelManager = levelPlugin.getClass().getMethod("getLevelManager").invoke(levelPlugin);
                        if (levelManager != null) {
                            levelManager.getClass().getMethod("setLevel", java.util.UUID.class, int.class)
                                .invoke(levelManager, deadplayer.getUniqueId(), 0);
                        }
                    } catch (Exception e) {
                        Bloodmoon.GetInstance().getLogger().warning("Could not reset custom level system: " + e.getMessage());
                    }
                }
            } catch (Exception e) {
                Bloodmoon.GetInstance().getLogger().warning("LevelPlugin not found or error accessing it: " + e.getMessage());
            }
        }

        // Remove 50% of balance if Vault is available
        try {
            org.bukkit.plugin.Plugin vaultPlugin = Bloodmoon.GetInstance().getServer().getPluginManager().getPlugin("Vault");
            if (vaultPlugin != null && vaultPlugin.isEnabled()) {
                try {
                    Class<?> economyClass = Class.forName("net.milkbowl.vault.economy.Economy");
                    var rsp = Bloodmoon.GetInstance().getServer().getServicesManager().getRegistration(economyClass);
                    if (rsp != null) {
                        Object economy = rsp.getProvider();
                        Method getBalanceMethod = economy.getClass().getMethod("getBalance", org.bukkit.OfflinePlayer.class);
                        Method withdrawMethod = economy.getClass().getMethod("withdrawPlayer", org.bukkit.OfflinePlayer.class, double.class);
                        
                        double balance = ((Number) getBalanceMethod.invoke(economy, deadplayer)).doubleValue();
                        if (balance > 0) {
                            double amountToRemove = balance * 0.5;
                            withdrawMethod.invoke(economy, deadplayer, amountToRemove);
                        }
                    }
                } catch (Exception e) {
                    Bloodmoon.GetInstance().getLogger().warning("Could not access Vault economy: " + e.getMessage());
                }
            }
        } catch (Exception e) {
            // Vault not available, silently continue
        }

        // Inventory clearing removed - we don't want that
    }
