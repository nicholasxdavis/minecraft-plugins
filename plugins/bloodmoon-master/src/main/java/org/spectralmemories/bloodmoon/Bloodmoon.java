...
public final class Bloodmoon extends JavaPlugin
{
    ...
    private void CheckOlderConfigs ()
    {
        ...
        if (oldConfig.exists()) getLogger().log(Level.WARNING,"[Deprecated] PandoraEclipse/config.yml is no longer used. Use per-world configuration instead");
        ...
        for (World world : bloodmoonWorlds)
        {
            ...
            if (getConfigReader(world).GetIntervalConfig() < 1)
                getLogger().log(Level.WARNING,"[Warning] SolarEclipseInterval config is set to 0 or less.\nThis may cause problem, please use the PermanentSolarEclipse option instead");
