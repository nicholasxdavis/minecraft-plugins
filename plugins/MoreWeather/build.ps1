# Build script for MoreWeather plugin
Write-Host "Building MoreWeather plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "MoreWeather-*.jar" | Select-Object -First 1
        if ($jarFile) {
