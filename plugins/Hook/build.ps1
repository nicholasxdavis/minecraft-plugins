# Build script for Hook plugin
Write-Host "Building Hook plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "Hook-*.jar" | Select-Object -First 1
        if ($jarFile) {
