# Build script for EssHats plugin
Write-Host "Building EssHats plugin..." -ForegroundColor Green
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path $pluginsPath) {
        $jarFile = Get-ChildItem -Path "target" -Filter "EssHats-*.jar" | Select-Object -First 1
        if ($jarFile) {
